...
@section('content_header')
    ...
    <h1>Detalhes do detalhe <b>({{ $detail->name }})</b></h1>
@stop
    ...
    <div class="card">
        ...
        <div class="card-footer">
            <form action="{{ route('details.plan.destroy', [$plan->url, $detail->id]) }}" method="post">
                @csrf
                @method('DELETE')
                <button type="submit" class="btn btn-danger"><i class="far fa-trash-alt"></i> Deletar o Detalhe <b>({{ $detail->name }})</b>, do plano  <b>({{ $plan->name }})</b></button>
            </form>
        </div>
    </div>
@endsection